    ...
    for (int phase = 0; phase < numtasks - 1; phase++) //começando a odd even so que dessa vez usando os processos
    {
        ...
        if (phase % 2 == 0) //fase par
        {
            ...
            else
            {
                ...
                MPI_Sendrecv(&local_v[local_size],local_size, MPI_INT, taskid, 0,local_v,local_size, MPI_INT, taskid, 0, MPI_COMM_WORLD, &status);  //enviando e recebendo o vetor dele mesmo, para colocar a parte que cabe a ele na primeir posição
